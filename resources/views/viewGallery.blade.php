   ...
   <body class="h-screen overflow-hidden" x-data="{ showModal: false, modalImage: '' }">
               ...
               <main>
                   <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
                       @foreach ($multimedia as $media)
                           <div class="relative rounded overflow-hidden shadow-lg bg-white cursor-pointer"
       ...
       <div x-show="showModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-75 z-50">
           <div class="bg-white p-4 rounded-lg shadow-lg max-w-3xl">
               <button @click="showModal = false" class="text-black font-bold py-2 px-4 rounded">Close</button>
               <img :src="modalImage" alt="Modal Image" class="mt-4 w-full">
           </div>
       </div>
   </body>
   </html>